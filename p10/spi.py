...
class Lexer(object):
    ...
    def number(self):
        """Return a (multidigit) number(integer or real) consumed from the input."""
        result = ''
...
class Var(AST):
    ...
    def __str__(self):
        return self.token.type + " " + self.value
    __repr__ = __str__
...
class Parser(object):
    ...
    def term(self):
        """term : factor ((MUL | FLOAT_DIV | REAL_DIV) factor)*"""
        node = self.factor()
