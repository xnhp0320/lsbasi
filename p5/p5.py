...
INT, MINUS, PLUS, MUL, DIV, LEFT, RIGHT, EOF = 1, 2, 3, 4, 5, 6, 7, 0
...
class Interpreter:
    ...
    def term(self):
        ...
        while self.current_token and self.current_token.type in [DIV, MUL]:
            token = self.current_token
            if token.type == DIV:
                self.eat(DIV)
    ...
    def expr(self):
        ...
        while self.current_token and self.current_token.type in [MINUS, PLUS]:
            token = self.current_token
            if token.type == MINUS:
                self.eat(MINUS)
                result = result - self.term()
